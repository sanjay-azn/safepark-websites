...
export default function HomePage() {
  ...
  return (
    <div className="scroll-smooth bg-black min-h-screen overflow-x-hidden">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            <div className="text-center lg:text-left space-y-6">
              ...
              <div className="flex flex-col sm:flex-row gap-4 pt-4 animate-fade-in-up" style={{ animationDelay: '0.3s' }}>
                <a href="tel:+919600840058" className="group relative px-8 py-3 rounded-xl bg-gradient-to-r from-emerald-500 via-emerald-400 to-cyan-500 text-white font-bold text-base hover:shadow-2xl hover:shadow-emerald-500/50 hover:scale-105 transition-all duration-500 inline-flex items-center justify-center gap-2 overflow-hidden">
                ...
                <a href="#gallery" className="relative px-8 py-3 rounded-xl border-2 border-emerald-400/60 bg-emerald-500/10 text-emerald-200 font-bold text-base hover:bg-emerald-500/20 hover:border-emerald-300 hover:text-emerald-100 transition-all duration-300 inline-flex items-center justify-center gap-2 group backdrop-blur-sm overflow-hidden">
      ...
      {/* GALLERY */}
      <section id="gallery" className="py-24 px-6 md:px-12 lg:px-20 relative bg-gradient-to-b from-slate-950 via-slate-900/20 to-black">
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div id="carousel" data-observe className="relative" style={{ opacity: isVisible['carousel'] ? 1 : 0, transform: isVisible['carousel'] ? 'scale(1)' : 'scale(0.95)', transition: 'all 0.8s cubic-bezier(0.34, 1.56, 0.64, 1)' }}>
            <div className="relative rounded-2xl overflow-hidden border border-emerald-400/30 bg-gradient-to-br from-gray-900 to-black shadow-2xl shadow-emerald-500/10 backdrop-blur-sm">
              ...
              <button onClick={() => setCurrentSlide((prev) => (prev - 1 + galleryImages.length) % galleryImages.length)} className="absolute left-6 top-1/2 -translate-y-1/2 p-3 rounded-full bg-gradient-to-r from-emerald-500/20 to-cyan-500/20 backdrop-blur-md text-white hover:from-emerald-500/30 hover:to-cyan-500/30 border border-emerald-400/30 transition-all z-10 hover:scale-110 duration-300 shadow-lg">
                <ChevronLeft className="w-6 h-6" />
              </button>
              <button onClick={() => setCurrentSlide((prev) => (prev + 1) % galleryImages.length)} className="absolute right-6 top-1/2 -translate-y-1/2 p-3 rounded-full bg-gradient-to-r from-emerald-500/20 to-cyan-500/20 backdrop-blur-md text-white hover:from-emerald-500/30 hover:to-cyan-500/30 border border-emerald-400/30 transition-all z-10 hover:scale-110 duration-300 shadow-lg">
            ...
            <div className="flex gap-4 mt-8 overflow-x-auto pb-2 scroll-smooth">
              {galleryImages.map((img, idx) => (
                <button key={idx} onClick={() => setCurrentSlide(idx)} className={`relative flex-shrink-0 w-28 h-28 rounded-xl overflow-hidden border-3 transition-all duration-300 ${currentSlide === idx ? 'border-emerald-400 ring-2 ring-emerald-400/50 scale-110 shadow-lg shadow-emerald-500/30' : 'border-white/20 hover:border-white/40'}`}>
      ...
      {/* FAQ */}
      <section className="py-24 px-6 md:px-12 lg:px-20 relative overflow-hidden bg-black">
        ...
        <div className="max-w-3xl mx-auto relative z-10">
          <div id="faq-header" data-observe className="text-center mb-16" style={{ opacity: isVisible['faq-header'] ? 1 : 0, transform: isVisible['faq-header'] ? 'translateY(0)' : 'translateY(30px)', transition: 'all 0.8s cubic-bezier(0.34, 1.56, 0.64, 1)' }}>
            ...
            {faqItems.map((item, idx) => {
              ...
              return (
                <div key={idx} id={faqId} data-observe className="border border-white/15 rounded-xl bg-gradient-to-br from-white/[0.08] to-white/[0.02] hover:from-white/[0.12] hover:to-white/[0.05] hover:border-emerald-400/40 backdrop-blur-sm transition-all duration-300 overflow-hidden shadow-lg" style={{ opacity: isFaqVisible ? 1 : 0, transform: isFaqVisible ? 'translateY(0)' : 'translateY(20px)', transition: `all 0.6s cubic-bezier(0.34, 1.56, 0.64, 1) ${idx * 100}ms` }}>
                  <button onClick={() => setExpandedFaq(expandedFaq === idx ? -1 : idx)} className="w-full px-6 py-5 flex items-center justify-between text-left hover:bg-gradient-to-r hover:from-emerald-500/5 hover:to-transparent transition-all">
                    ...
                    <div className={`ml-4 flex-shrink-0 p-1.5 rounded-lg ${expandedFaq === idx ? 'bg-emerald-500/20 text-emerald-300' : 'bg-white/10 text-gray-400'} transition-all duration-300`} style={{ transform: expandedFaq === idx ? 'rotate(180deg)' : 'rotate(0deg)' }}>
                      {expandedFaq === idx ? (<Minus className="w-5 h-5" />) : (<Plus className="w-5 h-5" />)}
                    </div>
                  </button>
                  {expandedFaq === idx && (<div className="px-6 pb-6 pt-2 text-gray-300 leading-relaxed border-t border-white/10 bg-gradient-to-r from-emerald-500/5 to-transparent" style={{ animation: 'fade-in-up 0.4s cubic-bezier(0.34, 1.56, 0.64, 1) forwards' }}>
                    {item.answer}
                  </div>)}
                </div>
              );
      ...
      {/* CTA */}
      <section className="py-24 px-6 md:px-12 lg:px-20 relative bg-gradient-to-b from-black via-slate-900/30 to-black">
        ...
        <div className="max-w-4xl mx-auto">
          <div id="cta" data-observe className="relative" style={{ opacity: isVisible['cta'] ? 1 : 0, transform: isVisible['cta'] ? 'scale(1)' : 'scale(0.95)', transition: 'all 0.8s cubic-bezier(0.34, 1.56, 0.64, 1)' }}>
            ...
            <div className="relative bg-gradient-to-br from-white/[0.08] to-white/[0.02] backdrop-blur-xl rounded-3xl p-12 border border-emerald-400/30 text-center space-y-6 shadow-2xl">
              ...
              <p className="text-lg text-gray-300 max-w-2xl mx-auto font-light">Join thousands who chose SafePark. Get started in minutes.</p>
        ...
        <div className="max-w-6xl mx-auto">
          <div className="grid md:grid-cols-2 gap-16 mb-12">
            <div>
              ...
              <p className="text-gray-400 leading-relaxed mb-6 font-light text-sm md:text-base">Premium car protection engineered for the modern world. Trusted by thousands across India.</p>
